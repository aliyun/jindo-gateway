...
public class JfsGetXAttrsResponse extends JfsAbstractHttpResponse {
    private static final Logger LOG = LoggerFactory.getLogger(JfsGetXAttrsResponse.class);

    private List<XAttr> xAttrs;

    public JfsGetXAttrsResponse() {
        super();
    ...
    public JfsStatus parseXml(String xml) {
        ...
        try {
            ...
            for (int i = 0; i < xAttrNodes.getLength(); i++) {
                ...
                String valueBase64 = JfsResponseXml.getNodeString(xAttrNode, "value", null, false);
                
                XAttr.NameSpace nameSpace = parseNameSpace(namespaceInt);
                
                byte[] value = null;
                if (valueBase64 != null && !valueBase64.isEmpty()) {
                    value = Base64.getDecoder().decode(valueBase64);
                }
            ...
            LOG.warn("Failed to parse getXAttrs response", e);
            return JfsStatus.ioError("Failed to parse getXAttrs response: " + e.getMessage());
        }
